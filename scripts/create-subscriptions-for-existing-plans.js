...
/**
 * Create subscriptions for existing active plans with valid user IDs
 */
...
async function createSubscriptionsForExistingPlans() {
  console.log('🔧 Creating Subscriptions for Existing Active Plans...\n')

  try {
    // Get active plans with valid user IDs
    ...
    console.log(`Found ${activePlans.length} active plans with valid user IDs and stripe subscription IDs`)

    if (activePlans.length === 0) {
      console.log('✅ No active plans need subscription creation')
      return
    }
    ...
    if (plansNeedingSubscriptions.length === 0) {
      console.log('✅ All active plans already have subscription records!')
      return
    }
    ...
    console.log('\n🎉 Subscription creation for existing plans completed!')
    console.log('\n📋 Summary:')
    console.log('✅ Created subscriptions for active plans with valid user IDs')
    console.log('❌ Ignored webhook events with invalid user IDs (old data)')
    console.log('\n🚀 Next steps:')
...
createSubscriptionsForExistingPlans()
