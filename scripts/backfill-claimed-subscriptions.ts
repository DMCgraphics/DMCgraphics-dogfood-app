...
async function backfillClaimedSubscriptions() {
  console.log("Finding claimed subscriptions without plans...")
  ...
  for (const sub of subscriptions) {
    try {
      ...
      console.log(`Linked subscription to plan and synced period dates`)
      ...
      if (recipeNames.length > 0) {
        ...
        if (recipes && recipes.length > 0) {
          ...
          console.log(`Created ${planItems.length} plan items`)
        }
      }
      ...
      console.log(`✓ Successfully backfilled subscription ${sub.id}`)

    } catch (error: any) {
      console.error(`✗ Failed to process subscription ${sub.id}:`, error.message)
    }
  }
...
backfillClaimedSubscriptions().catch(console.error)
