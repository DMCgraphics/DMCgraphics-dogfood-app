      ...
      try {
        ...
        if (dogsData && dogsData.length > 0) {
          if (subscriptionsData && subscriptionsData.length > 0) {
            ...
            hasActiveSub = true
            console.log("[v0] Found active subscriptions:", subscriptionsData)
          } else {
        ...
        const transformedDogs =
          dogsData?.map((dog) => {
            ...
            if (dogSubscription) {
              ...
              const subscriptionDate = new Date(dogSubscription.created_at)
              ...
              console.log("[v0] Active subscription found for", dog.name, "next delivery:", nextDelivery)
            } else {
              const savedPlanData = localStorage.getItem(`nouripet-saved-plan-${dog.id}`)
              if (savedPlanData) {
                try {
