...
interface PlanReviewProps {
  ...
  isCustomizingSubscription?: boolean
}
...
function PlanReviewInner({
  ...
  planType = "full",
  topperLevel = null,
  isCustomizingSubscription = false,
}: PlanReviewProps) {
  ...
  return (
    <div className="space-y-6">
          ...
          {displayRecipes.map((recipe, index) => (
            <div key={recipe.id} className={index > 0 ? "pt-4 border-t" : ""}>
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 dark:bg-blue-900 dark:text-blue-200">{recipe.aafcoLifeStage}</Badge>
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div>
            ...
            {isTopperPlan && displayRecipes.length > 0 && (
              <div className="text-xs text-muted-foreground mt-1 ml-2">
                {displayRecipes.length > 1 ? (
                  <>Recipes: {displayRecipes.map(r => r.name).join(", ")}</>
                ) : (
                  <>Recipe: {displayRecipes[0]?.name}</>
                )}
              </div>
            )}
          </div>
      ...
      <div className="space-y-4">
        {/* Debug logging for checkout button visibility */}
        {console.log("[v0] PlanReview - Checkout button visibility check:", {
          isAuthenticated,
          isLastDog,
          currentDogIndex,
          totalDogs,
          showButton: isAuthenticated && isLastDog
        })}

        {/* Multi-recipe disclaimer */}
        {displayRecipes.length > 1 && (
          <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <div className="flex gap-3">
              ...
              <div>
                <h4 className="text-sm font-medium text-blue-900 dark:text-blue-100">Recipe Variety Plan</h4>
                <p className="text-sm text-blue-800 dark:text-blue-200 mt-1">
                  Your order includes {displayRecipes.length} recipes for variety. The checkout page will show a single product name for billing purposes, but your subscription includes all {displayRecipes.length} recipes you selected.
                </p>
              </div>
            </div>
          </div>
